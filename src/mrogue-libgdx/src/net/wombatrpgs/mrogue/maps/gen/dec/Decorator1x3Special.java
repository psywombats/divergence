 ...
 */
public class Decorator1x3Special extends Decorator {
	...
	public void apply(Tile[][] tiles) {
		for (int x = 0; x < gen.getWidth(); x += 1) {
			for (int y = 0; y < gen.getHeight(); y += 1) {
				...
				if (!gen.isTile(tiles, mdo.bottomType, x, y-2)) continue;
				...
				tiles[y-1][x] = MGlobal.tiles.getTile(mdo.m);
				tiles[y-2][x] = MGlobal.tiles.getTile(mdo.b);
			}
		}
	}
